...
export default function CompanyManager() {
  ...
  const submit = () => {
    ...
    if (editId) {
      axios.put(`${API}/companies/${editId}`, payload).then(() => {
        alert("Updated");
        setEditId(null);
        setForm(emptyForm);
        loadCompanies();
      ...
      axios.post(`${API}/companies`, payload).then(() => {
        alert("Added");
        setForm(emptyForm);
        loadCompanies();
      });
  ...
  const deleteCompany = (id) => {
    if (window.confirm("Delete this company?")) {
      axios.delete(`${API}/companies/${id}`).then(() => loadCompanies());
    }
  };
        ...
        <div style={styles.formWrapper} ref={formRef}>
          <div style={styles.formBox}>
            ...
            <input
              ...
              onChange={(e) =>
                setForm({ ...form, coding_cutoff: e.target.value })
              }
            />
            ...
            <input
              ...
              onChange={(e) =>
                setForm({ ...form, aptitude_cutoff: e.target.value })
              }
            />
          ...
          <div style={styles.grid}>
            {companies.map((c) => (
              <div key={c._id} style={styles.card}>
                ...
                <p>
                  <b>Min CGPA:</b> {c.min_cgpa ?? "-"}
                </p>
                <p>
                  <b>Coding Cutoff:</b> {c.coding_cutoff ?? "-"}
                </p>
                <p>
                  <b>Aptitude Cutoff:</b> {c.aptitude_cutoff ?? "-"}
                </p>
                <p>
                  <b>Skills:</b> {c.skills?.length ? c.skills.join(", ") : "-"}
                </p>
