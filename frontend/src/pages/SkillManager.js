...
export default function SkillManager() {
  ...
  const submit = () => {
    ...
    if (editId) {
      axios.put(`${API}/skills/${editId}`, { name }).then(() => {
        setEditId(null);
        setName("");
        load();
      });
    } else {
      axios.post(`${API}/skills`, { name }).then(() => {
        setName("");
        load();
      });
    }
  ...
  return (
    <div>
      ...
      <div style={styles.page}>
        ...
        {skills.length === 0 && (
          <p style={styles.emptyText}>No skills available.</p>
        )}
            ...
            {skills.map((s) => (
              <div key={s._id} style={styles.card}>
                ...
                <div style={{ marginTop: 12 }}>
                  <button style={styles.buttonEdit} onClick={() => edit(s)}>
                    Edit
                  </button>
                  <button
